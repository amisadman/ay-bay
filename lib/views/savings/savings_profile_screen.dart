...
class _SavingsProfileScreenState extends State<SavingsProfileScreen> {
  ...
  void _showTransactionDialog(BuildContext context, FinanceProvider finProv, String type) {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setStateBuilder) => AlertDialog(
          title: Text(isAdd ? 'Add Money' : 'Retrieve Money', style: const TextStyle(color: AppColors.brown, fontWeight: FontWeight.bold)),
          content: Column(
        ...
        actions: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: isAdd ? AppColors.green : AppColors.red, foregroundColor: Colors.white),
            onPressed: () {
              final amount = double.tryParse(amountCtrl.text.trim()) ?? 0.0;
              if (amount > 0) {
                final date = DateTime.now().toIso8601String().split('T')[0];
                finProv.addSavingsTransaction(widget.savingsId, amount, type, date, logToTransactions: logToGlobal);
                Navigator.pop(ctx);
              }
            },
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Account Details', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 22)),
        actions: [
          IconButton(
                ...
                builder: (ctx) => AlertDialog(
                  ...
                  content: Column(
                    ...
                    children: [
                      ...
                      TextField(controller: acCtrl, decoration: const InputDecoration(labelText: 'A/C Number')),
                    ],
                  ),
                  actions: [
                    TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
          ...
          IconButton(
            ...
            onPressed: () {
              finProv.deleteSavingsAccount(acc.id!);
      ...
      body: Column(
        ...
        children: [
            ...
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    Text(acc.bankName.toUpperCase(), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white, letterSpacing: 1.5)),
                    const Icon(Icons.account_balance_rounded, color: Colors.white70),
                  ],
                ),
                ...
                Row(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('A/C Number', style: TextStyle(color: Colors.white54, fontSize: 12)),
                        const SizedBox(height: 4),
                        Text(acc.accountNumber, style: const TextStyle(color: Colors.white, fontSize: 16)),
                      ],
                    ),
                    if (acc.branchAddress.isNotEmpty)
                      Column(
                        ...
                        children: [
                          const Text('Branch', style: TextStyle(color: Colors.white54, fontSize: 12)),
                          const SizedBox(height: 4),
                          Text(acc.branchAddress, style: const TextStyle(color: Colors.white, fontSize: 16)),
                        ],
                      ),
                  ],
                )
              ...
              children: [
                Expanded(
                  child: ElevatedButton.icon(
                    ...
                    onPressed: () => _showTransactionDialog(context, finProv, 'add'),
                  ),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: ElevatedButton.icon(
                    style: ElevatedButton.styleFrom(backgroundColor: AppColors.red, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 14)),
                    icon: const Icon(Icons.remove_circle_outline),
                    label: const Text('Retrieve'),
                    onPressed: () => _showTransactionDialog(context, finProv, 'retrieve'),
                  ),
                ),
              ],
            ),
          ...
          const Padding(
            ...
            child: Text('Transaction History', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.brown)),
          ),
            ...
            child: transactions.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final trans = transactions[transactions.length - 1 - index];
                      ...
                      return ListTile(
                        leading: CircleAvatar(
                          backgroundColor: isAdd ? AppColors.green.withValues(alpha: 0.1) : AppColors.red.withValues(alpha: 0.1), 
                          child: Icon(isAdd ? Icons.arrow_downward : Icons.arrow_upward, color: isAdd ? AppColors.green : AppColors.red)
                        ),
                        title: Text(isAdd ? 'Deposit' : 'Withdrawal', style: const TextStyle(fontWeight: FontWeight.bold)),
                        subtitle: Text(date),
                        trailing: Text(
