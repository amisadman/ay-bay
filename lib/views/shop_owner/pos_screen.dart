...
class _POSScreenState extends State<POSScreen> {
  ...
  Future<void> _generateInvoice(List<SaleItem> items, double total, bool share,
      String shopName, double discount) async {
            ...
            children: [
              ...
              ...items.map((i) => pw.Padding(
                    ...
                    child: pw.Row(children: [
                      ...
                      pw.Expanded(
                          flex: 1,
                          child: pw.Text(i.quantity.toString(),
                              textAlign: pw.TextAlign.center,
                              style: const pw.TextStyle(fontSize: 10))),
                      pw.Expanded(
                          flex: 2,
                          child: pw.Text(
                              'Tk. ${(i.price * i.quantity).toStringAsFixed(2)}',
                              textAlign: pw.TextAlign.right,
                              style: const pw.TextStyle(fontSize: 10))),
                    ]),
                  )),
              ...
              pw.Row(
                ...
                children: [
                  ...
                  pw.Text('Tk. ${(total + discount).toStringAsFixed(2)}',
                      style: pw.TextStyle(
                          fontSize: 12, fontWeight: pw.FontWeight.bold)),
                ],
              ),
                  ...
                  children: [
                    pw.Text('DISCOUNT:',
                        style: pw.TextStyle(
                            ...
                            color: PdfColors.red)),
                    pw.Text('-Tk. ${discount.toStringAsFixed(2)}',
                        style: pw.TextStyle(
                            fontSize: 12,
                            fontWeight: pw.FontWeight.bold,
                            color: PdfColors.red)),
              ...
              pw.Row(
                ...
                children: [
                  ...
                  pw.Text('Tk. ${total.toStringAsFixed(2)}',
                      style: pw.TextStyle(
                          fontSize: 14, fontWeight: pw.FontWeight.bold)),
                ],
              ),
  ...
  void _showCheckoutDialog(List<ProductModel> products) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            'Sale recorded successfully. Would you like to generate a receipt?'),
        actions: [
          TextButton(
            onPressed: () {
              Navigator.pop(ctx);
  ...
  void _generateInvoiceOptions(List<ProductModel> products, bool share,
      {bool skipPdf = false}) async {
    ...
    await Provider.of<ShopOwnerProvider>(context, listen: false).recordSale(
        items,
        discount: _discount,
    ...
    if (!skipPdf) {
      await _generateInvoice(
          items,
          _calculateTotal(products) - _discount,
          share,
          Provider.of<ShopOwnerProvider>(context, listen: false).shopName ??
              'My Shop',
          _discount);
    }
                            ...
                            children: _cart.entries.map((e) {
                              ...
                              return ListTile(
                                title: Text(p.name,
                                    ...
                                    overflow: TextOverflow.ellipsis),
                                subtitle: Text(
                                    '৳${p.price.toStringAsFixed(2)} x ${e.value}'),
                                trailing: FittedBox(
                                  fit: BoxFit.scaleDown,
                                  child: Row(
                      ...
                      children: [
                        Expanded(
                          child: FittedBox(
                            ...
                            fit: BoxFit.scaleDown,
                            child: Text(
                                'Total: ৳${(total - _discount).toStringAsFixed(2)}',
                                style: const TextStyle(
                                    color: Colors.white,
                                    fontSize: 20,
                                    fontWeight: FontWeight.bold)),
