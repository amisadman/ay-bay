...
class ShopCloudService {
  ...
  Future<List<ProductModel>> getProducts() async {
    final snap =
        await _db.collection('shops').doc(shopId).collection('products').get();
    return snap.docs
  ...
  Future<List<CustomerModel>> getCustomers() async {
    final snap =
        await _db.collection('shops').doc(shopId).collection('customers').get();
    return snap.docs
  ...
  Future<List<EmployeeModel>> getEmployees() async {
    final snap =
        await _db.collection('shops').doc(shopId).collection('employees').get();
    return snap.docs
  ...
  Future<List<LedgerModel>> getLedger() async {
    final snap =
        await _db.collection('shops').doc(shopId).collection('ledger').get();
    return snap.docs
        .map((doc) => LedgerModel.fromMap(doc.data(), doc.id))
        .toList();
  }
}
